<script lang="ts">
	...
	$: {
		if (
			!isDesktopOrWider &&
			isViewportInLandscapeOrientation === false &&
			!isViewportOrientationPopupShownOnce
		) {
...
<main
	...
	<PortfolioSection
		images={[
			{ src: images.full.full2, alt: 'something random', objectFit: ObjectFit.SCALE_DOWN }
		]} />
	<PortfolioSection
		images={[
	...
	<PortfolioSection
		images={[
			{ src: images.full.full4, alt: 'something random', objectFit: ObjectFit.SCALE_DOWN }
		]} />
	<PortfolioSection
		images={[
